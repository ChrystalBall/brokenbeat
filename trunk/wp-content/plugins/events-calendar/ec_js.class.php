...
class EC_JS {
  ...
  function calendarData($m, $y) {
    ...
    ?>
        ...
        // <![CDATA[ 
          tb_pathToImage = "<?php bloginfo('siteurl');?>/wp-includes/js/thickbox/loadingAnimation.gif";
          tb_closeImage = "<?php bloginfo('siteurl');?>/wp-includes/js/thickbox/tb-close.png";    
          jQuery.noConflict();
          (function($) {
...
?>
    ...
    // <![CDATA[ 
      ...
      (function($) {        
        $(document).ready(function() {          
          $('#EC_previousMonth').append("&#171;<?php echo ucfirst($wp_locale->get_month_abbrev($wp_locale->get_month($m-1)));?>");
          $('#EC_nextMonth').prepend("<?php echo ucfirst($wp_locale->get_month_abbrev($wp_locale->get_month($m+1)));?>&#187;");          
          ...
          $('#EC_previousMonth').click(function() {            
            ...
            $.get("<?php bloginfo('siteurl');?>/index.php",
            {EC_action: "switchMonth", EC_month: <?php echo $m-1;?>, EC_year: <?php echo $y;?>},
            function(ecdata) {                            
              $('#calendar_wrap').empty();
              //$('#calendar_wrap').append(ecdata);              
              $('#calendar_wrap').append($(ecdata).html());
            });
          });
      ...
      foreach($this->db->getDaysEvents($sqldate) as $e) :
          ...
          if (($e->accessLevel == 'public') || (current_user_can($e->accessLevel))) {
            ...
            if ((!is_null($startTime) && !empty($startTime))) {
              list($ec_starthour, $ec_startminute, $ec_startsecond) = split(":", $startTime);
            ...
            if (($endTime != null) && (!empty($endTime))) {
              list($ec_endhour, $ec_endminute, $ec_endsecond) = split(":", $endTime);
            ...
            if (($endDate != null) && (!empty($endDate))) {
              list($ec_endyear, $ec_endmonth, $ec_endday) = split("-", $endDate);
          ...
          // <![CDATA[ 
            ...
            (function($) {
              $(document).ready(function() {
                $('#events-calendar-<?php echo $d;?>Large').append('<span class="event-block" id="events-calendar-<?php echo $id;?>Large"><?php echo $titlinked;?></span>');
...
?>
    <script type="text/javascript">
      ...
      (function($) {
        $(document).ready(function() {
          $('#EC_previousMonthLarge').append("&#171;<?php echo ucfirst($wp_locale->get_month($m-1));?>");
          $('#EC_nextMonthLarge').prepend("<?php echo ucfirst($wp_locale->get_month($m+1));?>&#187;");
          /*---------------------------------------------------- */
          ...
          $('#EC_previousMonthLarge').click(function() {
            ...
            function(ecdata) {
              ...
              //$('#calendar_wrapLarge').append(ecdata);
              $('#calendar_wrapLarge').append($(ecdata).html());
            });
          });
