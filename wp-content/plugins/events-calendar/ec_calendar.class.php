...
class EC_Calendar {
   ...
   */
    ...
    $first_of_month = gmmktime(0,0,0,$month,1,$year);
	  ...
	  $weekday = ($weekday + 7 - $first_day) % 7; //adjust for $first_day
		...
		$calendar = '<div id="calendar_wrap"><table summary="Event Calendar" id="wp-calendar"><caption id="calendar-month" class="calendar-month">'.$title.'</caption><thead><tr>';
  ...
  function displayLarge($year, $month, $days = array(), $day_name_length = 7) {
    ...
    $calendar .= '<table summary="Large Event Calendar" id="wp-calendarLarge">'."\n".'<thead><tr>'."\n";
    ...
    $first_of_month = gmmktime(0,0,0,$month,1,$year);
	  ...
	  $weekday = ($weekday + 7 - $first_day) % 7; //adjust for $first_day
		...
		$calendar = '<h2 style="text-align:center;"><a href="?page=events-calendar&amp;EC_action=switchMonthAdmin&amp;EC_month='.($month-1).'&amp;EC_year='.($year).'">&#171; '.$previousMonth.'</a>  '. __('Events','events-calendar') .' ('.$title.')  <a href="?page=events-calendar&amp;EC_action=switchMonthAdmin&amp;EC_month='.($month+1).'&amp;EC_year='.($year).'">'.$nextMonth.' &#187;</a></h2><hr />';
