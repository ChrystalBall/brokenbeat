<?php
/*
	modified bbcode plugin for WP-Froum, just tagged ff_ and FF_ on stuff
...
if (!defined('FF_CUSTOM_TAGS')) {
    define('FF_CUSTOM_TAGS', TRUE);
}
...
class FF_HTML_BBCodeParser
{
    ...
    * &FF_HTML_BBCodeParser::getStaticProperty(), overwriting them with (if present)
    ...
    * then stored back with &FF_HTML_BBCodeParser::getStaticProperty(), so that the filter
    ...
    */
    function FF_HTML_BBCodeParser($options = array())
    {

        /* set the already set options */
        $baseoptions = &FF_HTML_BBCodeParser::getStaticProperty('FF_HTML_BBCodeParser', '_options');
        ...
        if (is_subclass_of($this, 'FF_HTML_BBCodeParser')) return;

        /* extract the definedTags from subclasses */
        $filters = explode(',', $this->_options['filters']);
        foreach ($filters as $filter) {
            $class = 'FF_HTML_BBCodeParser_Filter_'.$filter;
...
class FF_HTML_BBCodeParser_Filter_Links extends FF_HTML_BBCodeParser
{
    ...
    */
    function _preparse()
    {
        $options = FF_HTML_BBCodeParser::getStaticProperty('FF_HTML_BBCodeParser','_options');
        $o = $options['open'];
        $c = $options['close'];
        $oe = $options['open_esc'];
...
class FF_HTML_BBCodeParser_Filter_Lists extends FF_HTML_BBCodeParser
{
...
class FF_HTML_BBCodeParser_Filter_Images extends FF_HTML_BBCodeParser
{
...
class FF_HTML_BBCodeParser_Filter_Extended extends FF_HTML_BBCodeParser
{
...
class FF_HTML_BBCodeParser_Filter_Email extends FF_HTML_BBCodeParser
{
...
class FF_HTML_BBCodeParser_Filter_Basic extends FF_HTML_BBCodeParser
{
...
function FF_BBCode($text) {
	global $FF_PARSER_OBJECT;

	if ($FF_PARSER_OBJECT==""){
		$FF_PARSER_OBJECT =& new FF_HTML_BBCodeParser(array(
			'quotestyle'    => 'single',
			'quotewhat'     => 'all',
			'open'          => '[',
			'close'         => ']',
	...
	return $FF_PARSER_OBJECT->qparse($text);
}
